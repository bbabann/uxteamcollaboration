...
> v1.1 추가분. 아래 내용은 기존 §5(내비게이션), §6(페이지 구조), §7(필터), §9(컴포넌트 매핑)의 정적 규격에 **동작**을 부여합니다. 토큰·컴포넌트 자체는 바뀌지 않습니다.

### 14.0 공통 원칙

- **호버 = 신호, 클릭 = 확정.** 마우스가 스치기만 해도 상태가 바뀌는 요소는 만들지 않습니다. 호버는 "누를 수 있다"는 걸 보여주는 배경색·커서 변화까지만 담당하고, 실제 펼침·전환·선택은 항상 클릭(또는 키보드 Enter/Space)으로 확정합니다. 의도치 않은 오작동을 막기 위한 규칙입니다.
- **모든 인터랙티브 요소는 키보드로도 조작 가능해야 합니다.** Tab으로 이동, Enter/Space로 실행, 드롭다운은 화살표 키로 항목 이동.
- **트랜지션은 두 속도만 사용합니다.**

| 토큰 | 값 | 용도 |
|---|---|---|
| `--duration-fast` | 150ms ease-out | 배경색 변화, 체크박스, 작은 상태 전환 |
| `--duration-base` | 200ms ease-out | 펼침/접힘 높이 변화, 탭 콘텐츠 전환 |

이 외의 임의 속도(예: 300ms, 500ms)는 사용하지 않습니다.
...
- **트리거**
- 그룹 헤더 클릭 → 펼침/접힘 토글. 호버만으로는 토글되지 않습니다.
- 한 번에 여러 그룹을 펼쳐둘 수 있습니다 (아코디언처럼 하나만 열리는 방식 아님). 사용자가 여러 섹션을 동시에 참고하는 대시보드 특성상, 클릭할 때마다 다른 그룹이 닫히면 방해가 됩니다.

**상태별 표현**

| 상태 | 배경 | 텍스트 | Chevron |
|---|---|---|---|
| 기본 | 투명 | `--sidebar-foreground` | 0° |
| 호버 (신호만) | `rgba(255,255,255,0.06)` | `--sidebar-foreground` | 0° |
| 펼쳐짐 (클릭 후 확정) | `--neutral-strong` | `#FFFFFF` | 180° |
| 포커스(키보드) | 위 상태 + `outline: 2px solid var(--primary)` | — | — |

> 호버와 "펼쳐짐"을 다른 배경으로 구분한 이유: 같은 색이면 사용자가 "지금 열린 상태인지, 그냥 마우스가 올라가 있는 건지" 구분할 수 없습니다.

**동작**
- 펼침/접힘은 높이(`max-height`) 애니메이션, `--duration-base`.
- Chevron 회전은 `--duration-fast`.
- 하위 항목 클릭 → 해당 페이지로 이동, 그 그룹은 펼쳐진 채로 유지.

---

### 14.2 필터 드롭다운 (Brand / Country / Store)

**트리거**
- 셀렉트 박스 클릭 → 옵션 목록 열림. 호버로 열리지 않습니다 (호버로 여는 드롭다운은 실수로 다른 옵션이 눌리는 사고를 유발합니다).
...
**동작**
- 옵션 목록은 트리거 바로 아래, `--radius-md`, 그림자 사용(§4.4 예외 — 떠 있는 레이어이므로 허용).
- 목록이 6개를 넘으면 스크롤. 새로 스크롤 영역을 만들지 않고 목록 자체가 스크롤됩니다.
- 선택된 항목은 `--primary-subtle` 배경 + 좌측 체크 아이콘.
- 닫힘 조건: 항목 선택 / 바깥 클릭 / `Esc` 키.
- 옵션 진입/퇴장 애니메이션은 `--duration-fast`.
- 키보드: 위/아래 화살표로 항목 이동, 문자 입력 시 해당 글자로 시작하는 항목으로 점프(type-ahead).

---

### 14.3 브레드크럼 하위 탭 (Session / Comparison 등)

**용어 정리** — 번호를 매겨 페이지를 넘기는 페이지네이션이 아니라, 클릭 시 아래 콘텐츠 영역 전체가 바뀌는 **탭 전환**입니다. §6 페이지 구조에서 이미 `tabs` 컴포넌트로 규격화된 부분에 동작을 추가하는 것입니다.

**트리거**
- 탭 클릭 → 콘텐츠 전환. 호버로 전환되지 않습니다 (마우스가 지나가다 콘텐츠가 바뀌면 읽던 내용을 잃게 됩니다).

**상태별 표현**

| 상태 | 텍스트 색 | 밑줄 |
|---|---|---|
| 기본 | `--muted-foreground` | 없음 |
| 호버 (신호만) | `--foreground` | 없음 |
| 활성 | `--primary` | 2px, `--primary` |

**동작**
- 밑줄은 탭 사이를 이동할 때 위치가 슬라이드되며 이동, `--duration-base`.
- 콘텐츠 영역은 교체 시 8px 아래에서 위로 살짝 올라오며 페이드인 (`--duration-base`). 로딩 스피너 없이 즉시 전환 가능한 경우 페이드만 적용합니다.
- 브라우저 주소(URL)에 현재 탭 상태를 반영해, 새로고침해도 같은 탭이 유지되게 합니다.
- 탭 전환은 같은 페이지 안에서 이루어지며 전체 페이지가 다시 로드되지 않습니다.

---

### 14.4 §9 매핑표 보강

기존 §9 표의 아래 항목에 인터랙션 규격이 추가로 적용됩니다. 컴포넌트 자체는 변경 없습니다.

| 화면 요소 | 참조 |
|---|---|
| 메뉴 그룹 펼침 (`collapsible`) | §14.1 |
| Brand/Country/Store (`select`) | §14.2 |
| 상단 탭 (`tabs`) | §14.3 |

---

### 14.5 금지 사항 추가 (§10에 병합)

- ❌ 호버만으로 메뉴 펼침, 드롭다운 열림, 탭 전환이 일어나게 하지 않습니다.
- ❌ `--duration-fast` / `--duration-base` 외의 임의 트랜지션 속도를 사용하지 않습니다.
- ❌ 키보드로 도달·조작할 수 없는 인터랙티브 요소를 만들지 않습니다.
